...
| Java 17 | Java 11 ⚠️ เวอร์ชันไม่ตรง! |
| มี library X ติดตั้งไว้แล้ว | ไม่มี library X ⚠️ ขาดของ! |
...
```mermaid
flowchart TB
    subgraph Plain["รันแบบธรรมดา (ไม่มี Docker)"]
        direction TB
        OS["OS เดียวกัน — แชร์ library ร่วมกัน ชนกันได้"]
        A1["โปรแกรม A"] --- OS
        B1["โปรแกรม B"] --- OS
    end
    subgraph WithDocker["รันด้วย Docker"]
        direction LR
        A2["โปรแกรม A<br/>+ ของที่ A ต้องการ"]
        B2["โปรแกรม B<br/>+ ของที่ B ต้องการ"]
    end
```

ฝั่งซ้าย: โปรแกรม A กับ B ใช้ OS ร่วมกัน แชร์ library เดียวกัน ชนกันได้ — ฝั่งขวา: แต่ละโปรแกรมมีของที่ต้องการเป็นของตัวเอง แยกกันเด็ดขาด
...
```mermaid
flowchart LR
    Img["nginx:latest (image)"] --> C1["container 1 (กำลังรันเว็บ A)"]
    Img --> C2["container 2 (กำลังรันเว็บ B)"]
    Img --> C3["container 3 (กำลังรันเว็บ C)"]
```
