...
1. **เริ่มต้น** — เข้าใจว่า Redis ไม่ใช่ key-value ธรรมดา แต่มี data structure หลายแบบให้เลือกใช้ (ข้อ 2) — เลือก structure ให้ตรงกับรูปร่างของปัญหาตั้งแต่แรกสำคัญกว่าจำคำสั่งให้ได้เยอะ
2. **ลงมือทำจริง** — รัน Redis ผ่าน `docker run -p 6379:6379 redis` แล้วลองทำ **cache-aside pattern** (ข้อ 4.1) กับ endpoint ที่มีอยู่แล้วในโปรเจกต์เดิม — เห็นความต่างของความเร็วก่อน-หลังชัดเจนที่สุด
3. **ใช้งานได้คล่อง** — เข้าใจ persistence (RDB/AOF, ข้อ 3) ว่าเมื่อไหร่ต้องเปิด, รู้จัก cache stampede และวิธีป้องกัน, ตั้ง eviction policy ให้เหมาะกับงาน, และรู้ขอบเขตของ Pub/Sub (ข้อความหายได้ถ้าไม่มีคนฟัง) ก่อนเอาไปใช้กับงานที่ข้อมูลห้ามหาย
